...
public interface Graph {
    ...
    default ArrayList<Integer> result(int from, int end) {
        boolean[] visited = new boolean[vertexCount()];
        ...
        if (!flag[0]) {
            ...
            boolean[] visited2 = new boolean[vertexCount()];
            final boolean[] flag2 = {false};
            final Integer[] uz = new Integer[1];
            boolean[] finalVisited = visited2;
            class Inner2 {
                ;

                void visit2(Integer curr) {
                    if (curr == end) {
                        flag2[0] = true;
                    }
            ...
            for (int i = 0; i < vertexCount(); i++) {
                if (!(i == from || i == end)) {
                    {
                        visited2[i] = true;
                        new Inner2().visit2(from);
                        if (!flag2[0]) {
                            isk.add(i);
                        }
                        flag2[0] = false;
                        for (int x = 0; x < vertexCount(); x++) {
                            finalVisited[x] = false;
                        }
                    }
                }
            }
